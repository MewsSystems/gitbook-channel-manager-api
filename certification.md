...
| **Receive multi-space, multi-rate rate push** | Required | 5+ Spaces (3 Rooms, 1 Dorm and 1 Bed) and 2 Rates are mapped and pushed the [Update Prices](channel-manager-api.md#update-prices) endpoint. | The channel manager returns ```{ "success": true}``` and data is updated in the channel manager extranet. | Mapping bed and dorm space types is recommended, not required. |
